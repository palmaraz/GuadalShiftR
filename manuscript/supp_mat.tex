...
\begin{equation}\label{StochCuspModel_2}
	dX_{t} = (-X(t)^3 + \beta X(t) + \alpha)dt + \sigma_{x} dW(t)
\end{equation}
where the term within brackets, $-X(t)^3 + \beta X(t) + \alpha $, is the drift term and $ \sigma_{x} $ is the diffusion parameter (variance). The solution of Eqn. \ref{StochCuspModel_2} satisfies the Fokker–Planck equation \cite{Zeeman1988}. While not solvable analytically, there is an associated stationary density function, $\pi (x)$, that can be easily solved. Given a constant $\phi$, this density has the form:

\begin{equation}\label{CuspStatDensity}
	\pi (x) = \phi \text{exp} \left\{ \frac{2}{\sigma_{x}^2} \left(-\frac{1}{4} x^4 + \frac{1}{2}\beta x^2 + \alpha x \large\right)\right\}
\end{equation}
The model in \ref{CuspStatDensity} reduces to a maximum-likelihood problem if data for fitting $ \alpha $, $ \beta $ and $ X $ is available (see \cite{Cobb1985,Grasman2009} for details). In the accompanying paper we fit the stochastic cusp catastrophe model to the wintering waterfowl community time series of Doñana wetlands using the \verb|R| package \verb|cusp| \cite{Grasman2009}. Specifically the empirical model is: \\
